    ...
    try {
      ...
      getConnection((err, connection) => {
        ...
        connection.query(selectQuery, [email], (err, results) => {
          ...
          if (results[0].status === 'deactivate') {
            res.status(401).json({
              message: "Account is deactivated. Reactivate to login.",
              result: false,
            });
            return;
          }
      ...
      res.status(500).json({
        message:
          "There was a problem in registering the user, please try again.",
        result: false,
      });
    }
  },
  ...
  signOut: async function (req, res, next) {
    try {
      getConnection((err, connection) => {
        ...
        const deleteAuthEntryQuery = "DELETE FROM auth_tokens WHERE token = ?";
